...
U_train = np.array([0.05,0.1,0.5,5,20,50])
...
np.savetxt(f'./time/M-{M}-N-{N}-U_train-{U_train}-lr-{lr}.csv',[tf-t0])


#Saving loss_history
np.savetxt(f'./loss-history/M-{M}-N-{N}-U_train-{U_train}-lr{lr}.csv',Loss_history)
...
#Saving testing time
np.savetxt(f'./time/M-{M}-N-{N}-U_test-{U_train}-lr-{lr}.csv',[tf1-t1])

#Saving ground state energies for training and testing
np.savetxt(f'./E_gs/M-{M}-N-{N}-U_train-{U_train}-lr{lr}.csv',np.c_[U_train,E_train])
np.savetxt(f'./E_gs/M-{M}-N-{N}-U_test-{U_train}-lr{lr}.csv',np.c_[U_test,E_test])


#Saving ground states for training and testing
if os.path.exists(f'./states/M-{M}-N-{N}-U_train-{U_train}-lr-{lr}')==False:
    os.mkdir(f'./states/M-{M}-N-{N}-U_train-{U_train}-lr-{lr}')
for i in range (len(U_train)):
    gs_train=wf_gs[i*model.tot_states:(i+1)*model.tot_states].squeeze().double()
    np.savetxt(f'./states/M-{M}-N-{N}-U_train-{U_train}-lr-{lr}/U_train-{U_train[i]}.csv',gs_train.cpu().detach().numpy())
    #np.savetxt(path1+f'/states/M-{M}-N-{N}-U_train-{U_train}-lr-{lr}/U_train-{U_train[i]}-adam.csv',gs_train.cpu().detach().numpy())
    
if os.path.exists(f'./states/M-{M}-N-{N}-U_test-{U_train}-lr-{lr}')==False:
    os.mkdir(f'./states/M-{M}-N-{N}-U_test-{U_train}-lr-{lr}')
for i in range (len(U_test)):
    gs_test=wf_gs_test[i*model.tot_states:(i+1)*model.tot_states].squeeze().double()
    np.savetxt(f'./states/M-{M}-N-{N}-U_test-{U_train}-lr-{lr}/U_test-{U_test[i]}.csv',gs_test.cpu().detach().numpy())
