...
def absenergy_1state(wf_all, H_list, n_state_list, mu_list, U_list, t, N_list, V, M):
    
    n_avg_states=50
    target_epsilon=0.5
    E = torch.zeros(len(U_list)*len(N_list)*n_avg_states)
    idx = 0
...
#            vals_exact=np.genfromtxt(f'../../E_excited_exact/M-{M}-N-{N}-U-{U}.csv')
#            energy_min=min(vals_exact)
#            energy_max=max(vals_exact)
#            target_energy=energy_min+target_epsilon*(energy_max-energy_min)
#            ene_idx=np.argmin(abs(vals_exact-target_energy))
            vals_exact=np.genfromtxt(f'../../1D-excited-states/E_excited_exact/M-{M}-N-{N}-U-{U}.csv')
            energy_min=min(vals_exact)
            energy_max=max(vals_exact)
            target_energy=energy_min+target_epsilon*(energy_max-energy_min)
            #ene_idx=np.argmin(abs(vals_exact-target_energy))
            ene_idx=np.argsort(abs(vals_exact-target_energy))
            for kk in ene_idx[0:n_avg_states]:
                E[idx] = abs(E_tmp/torch.sqrt(torch.sum(wf_here.pow(2)))-vals_exact[kk])
                idx += 1
...
def overlap_1state(M, wf_all, H_list, n_state_list, mu_list, U_list, t, N_list, V):
...
#            gs_vecs = np.genfromtxt(f'../../1D-excited-states/exact_states/M-{M}-N-{N}/{U}.csv')
            
            if U==5.0 or U==20.0 or U==50.0:
                U=int(U)
            vals_exact=np.genfromtxt(f'../../1D-excited-states/E_excited_exact/M-{M}-N-{N}-U-{U}.csv')
            energy_min=min(vals_exact)
            energy_max=max(vals_exact)
            ...
            ene_idx=np.argsort(abs(vals_exact-target_energy))
            for kk in ene_idx[0:n_avg_states]:
                gs_vecs = np.genfromtxt(f'../../1D-excited-states/excited_states_exact/M-{M}-N-{N}/U-{U}-state-{kk}.csv')
                gs_vecs_t = torch.from_numpy(gs_vecs)
                E_tmp = torch.dot(gs_vecs_t, wf_here) 
                E[idx] = E_tmp
                idx += 1
...
def fractal_dim_diff_1state(M, wf_all, H_list, n_state_list, mu_list, U_list, t, N_list, V, q):
    ...
    for (j,N) in enumerate(N_list):
        ...
        for (i,U) in enumerate(U_list):
            ...
            for kk in ene_idx[0:n_avg_states]:
                gs_vecs = np.genfromtxt(f'../../1D-excited-states/excited_states_exact/M-{M}-N-{N}/U-{U}-state-{kk}.csv')
                #gs_vecs = torch.from_numpy(gs_vecs)

                if q==1:
                   frac_dim = -torch.sum(torch.pow(torch.abs(wf_here),2*q)*torch.log(torch.pow(torch.abs(wf_here),2*q)))
                   #gs_vecs = np.genfromtxt(f'../../1D-excited-states/exact_states/M-{M}-N-{N}/{U}.csv')
                   gs_frac = -np.sum(np.power(np.abs(gs_vecs),2*q)*np.log(np.power(np.abs(gs_vecs),2*q)))
                   E_tmp = abs(frac_dim-gs_frac)
                   E[idx] = E_tmp
                   idx += 1

                elif q>=1e5:
                    frac_dim = -torch.log(torch.max(torch.pow(torch.abs(wf_here),2)))
                    #gs_vecs = np.genfromtxt(f'../../1D-excited-states/exact_states/M-{M}-N-{N}/{U}.csv')
                   ...
                   frac_dim = 1/(1-q)*torch.log(torch.sum(torch.pow(torch.abs(wf_here),2*q)))
                   #gs_vecs = np.genfromtxt(f'../../1D-excited-states/exact_states/M-{M}-N-{N}/{U}.csv')
...
def train_NN(model_list, N_list, mu_list, U_list, t, c1, c2, c3, qlist, V, S, params, fname, 
    ...
    betas = [0.999, 0.9999]    
    # betas = [0.9, 0.99]
    #optimizer = optim.RMSprop(fc0.parameters(), lr=lr, alpha=0.99)  
    ...
    if not es_only: 
        ...
        if ((load_states and all(cond)) or n_excited==1 or load_states_indv == [0]) and (path.exists(fname + "gs")): 
            ...
            #u = fc_gs(all_states.double(),Ut_tensor.double())[:,0]
            #u = torch.exp(torch.pow(fc_gs(all_states.double(),Ut_tensor.double())[:,0],3))
            u = torch.exp(torch.mul(torch.sgn(fc_gs(all_states.double(),Ut_tensor.double())[:,0]), torch.pow(fc_gs(all_states.double(),Ut_tensor.double())[:,0],2)))
            ...
            #wf_gs = torch.exp(u[:,0])
            #wf_gs = u[:,0]
            #wf_gs = torch.exp(torch.pow(u[:,0],3))
            wf_gs = torch.exp(torch.mul(torch.sgn(u[:,0]), torch.pow(u[:,0],2)))
        ...
        for idx in range(steps):
            if not stop: 
                ...
                if gs_opt: 
                    #wf_re = torch.exp(u[:,0])
                    #wf_re = u[:,0]
                    #wf_re = torch.exp(torch.pow(u[:,0],3))
                    wf_re = torch.exp(torch.mul(torch.sgn(u[:,0]), torch.pow(u[:,0],2)))
                ...
                if not gs_opt: # excited satate
                    ...
                    if use_gpu: 
                        with torch.cuda.amp.autocast():
                            ...
                            #Ltot = (1-cc)*energy_1state(wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V)+cc*(1-overlap_1state(M, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V))
                            Ltot = c1*energy_1state(wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V)+c2*(1-overlap_1state(M, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V))
                            for q in qlist:
                                Ltot = Ltot+c3*fractal_dim_diff_1state(M, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, q)/np.log(model.tot_states)/len(qlist)
                            etot = energy_1state(wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V)
                            ovtot = overlap_1state(M, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V)
                            fractot0 = fractal_dim_diff_1state(M, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[0])/np.log(model.tot_states)
                            fractot1 = fractal_dim_diff_1state(M, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[1])/np.log(model.tot_states)
                            fractot2 = fractal_dim_diff_1state(M, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[2])/np.log(model.tot_states)
                            fractot3 = fractal_dim_diff_1state(M, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[3])/np.log(model.tot_states)
                            fractot4 = fractal_dim_diff_1state(M, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[4])/np.log(model.tot_states)
                            #fractot5 = fractal_dim_diff_1state(M, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[5])/np.log(model.tot_states)
                        Loss_history.append(Ltot.data.detach().cpu())  
                        energy_history.append(etot.data.detach().cpu())  
                        overlap_history.append(ovtot.data.detach().cpu())  
                        frac_dim_history0.append(fractot0.data.detach().cpu())
                        frac_dim_history1.append(fractot1.data.detach().cpu())
                        frac_dim_history2.append(fractot2.data.detach().cpu())
                        frac_dim_history3.append(fractot3.data.detach().cpu())
                        frac_dim_history4.append(fractot4.data.detach().cpu())
                        #frac_dim_history5.append(fractot5.data.detach().cpu())
                        ...
                        #fractot5 = fractal_dim_diff_1state(M, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[5])/np.log(model.tot_states)
                        Loss_history.append(Ltot.data.detach())  
                        energy_history.append(etot.data.detach())  
                        overlap_history.append(ovtot.data.detach())  
                        frac_dim_history0.append(fractot0.data.detach())
                        frac_dim_history1.append(fractot1.data.detach())
                        frac_dim_history2.append(fractot2.data.detach())
                        frac_dim_history3.append(fractot3.data.detach())
                        frac_dim_history4.append(fractot4.data.detach())
                        #frac_dim_history5.append(fractot5.data.detach())
...
#                                                                                                    grad_tot))
                ...
                if (iteration % loss_check_steps == 0) and (iteration > 0): 
                    ...
                    if int(iteration / loss_check_steps) > 2:
                        ...
                        #stop6 = (np.min(np.abs(loss_local)) < loss_diff)
                        stop6 = (loss_local[-1] < loss_diff)

                        stop7 = (loss_local[-1]-loss_local[-2] < 0)

                        #stop8 = (np.abs(loss_local[-1]-loss_local[-2]) < loss_diff)
                        
                        if (stop2 and stop3) or stop4 or stop5: 
                        #if stop5: 
                        #if (stop6 and stop7) or stop4: 
                            min_idx = np.argmin(loss_list)
                            fc1 = copy.deepcopy(nn_list[min_idx])
                            print(np.min(loss_list))
    ...
    return fc1, Loss_history, energy_history, overlap_history, frac_dim_history0, frac_dim_history1, frac_dim_history2, frac_dim_history3, frac_dim_history4, dot_history, all_E_list
...
def wf_e_calc(model, N, U_list, mu_list, t, V, n_excited, load_states_indv, fc_gs, fc1=None, fc2=None, use_gpu=False): 
    ...
    #wf_gs = fc_gs(all_states.double(),Ut_tensor.double())
    #wf_gs = torch.exp(torch.pow(fc_gs(all_states.double(),Ut_tensor.double()),3))
    wf_gs = torch.exp(torch.mul(torch.sgn(fc_gs(all_states.double(),Ut_tensor.double())), torch.pow(fc_gs(all_states.double(),Ut_tensor.double()),2)))
