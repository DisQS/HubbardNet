...
def fractal_dim_diff_1state(M, wf_all, H_list, n_state_list, mu_list, U_list, t, N_list, V, q):
    ...
    for (j,N) in enumerate(N_list):
        ...
        for (i,U) in enumerate(U_list):
            ...
            frac_dim = 1/(1-q)*torch.log(torch.sum(torch.pow(wf_here,2*q)))
            gs_vecs = np.genfromtxt(f'./exact_states/M-{M}-N-{N}/{U}.csv')
...
def train_NN(model_list, N_list, mu_list, U_list, t, cc, V, S, params, fname, 
...
#                                                                                                    grad_tot))
    ...
    return fc1, Loss_history, energy_history, overlap_history, dot_history, all_E_list
