...
#            ene_idx=np.argmin(abs(vals_exact-target_energy))
            vals_exact=np.genfromtxt(f'./E_excited_exact/M-{M}-N-{N}-w-{w}-h-{h}-U-{U}.csv')
            energy_min=min(vals_exact)
            energy_max=max(vals_exact)
            target_energy=energy_min+target_epsilon*(energy_max-energy_min)
            #ene_idx=np.argmin(abs(vals_exact-target_energy))
...
def overlap_1state(M,w,h, wf_all, H_list, n_state_list, mu_list, U_list, t, N_list, V):
...
#            gs_vecs = np.genfromtxt(f'./exact_states/M-{M}-N-{N}/{U}.csv')
            ...
            ene_idx=np.argsort(abs(vals_exact-target_energy))
            for kk in ene_idx[0:n_avg_states]:
                gs_vecs = np.genfromtxt(f'./excited_states_exact/M-{M}-N-{N}-w-{w}-h-{h}/U-{U}-state-{kk}.csv')
...
def fractal_dim_diff_1state(M,w,h, wf_all, H_list, n_state_list, mu_list, U_list, t, N_list, V, q):
...
def train_NN(model_list,w,h, N_list, mu_list, U_list, t, c1, c2, c3, qlist, V, S, params, fname, 
                ...
                if not gs_opt: # excited satate
                    ...
                    if use_gpu: 
                        with torch.cuda.amp.autocast():
                            ...
                            #Ltot = (1-cc)*energy_1state(wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V)+cc*(1-overlap_1state(M,w,h, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V))
                            Ltot = c1*energy_1state(wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V)+c2*(1-overlap_1state(M,w,h, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V))
                            for q in qlist:
                                Ltot = Ltot+c3*fractal_dim_diff_1state(M,w,h, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, q)/np.log(model.tot_states)/len(qlist)
                            etot = energy_1state(wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V)
                            ovtot = overlap_1state(M,w,h, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V)
                            fractot0 = fractal_dim_diff_1state(M, w, h, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[0])/np.log(model.tot_states)
                            fractot1 = fractal_dim_diff_1state(M, w, h, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[1])/np.log(model.tot_states)
                            fractot2 = fractal_dim_diff_1state(M,w, h, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[2])/np.log(model.tot_states)
                            fractot3 = fractal_dim_diff_1state(M,w, h, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[3])/np.log(model.tot_states)
                            fractot4 = fractal_dim_diff_1state(M,w, h, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[4])/np.log(model.tot_states)
                            #fractot5 = fractal_dim_diff_1state(M,w,h, wf_re, H_list, tot_states_list, mu_list, U_perturb, t, N_list, V, qlist[5])/np.log(model.tot_states)
                        Loss_history.append(Ltot.data.detach().cpu())  
